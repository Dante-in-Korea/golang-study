...
func TestInsertAfter(t *testing.T) {
	...
	assert.Equal(t, 4, l.GetAt(2).Value) //Elrimental -> 4
	assert.Equal(t, 3, l.Back().Value)   // Last Elimental
...
func TestInsertBefore(t *testing.T) {
	...
	assert.Equal(t, 4, l.Count2())       // the whole length
	assert.Equal(t, 4, l.GetAt(1).Value) //Elrimental -> 4
	assert.Equal(t, 2, l.GetAt(2).Value) // Last Elimental
	assert.Equal(t, 3, l.Back().Value)   // Last Elimental
